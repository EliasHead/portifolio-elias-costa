...
import { WorkExperience } from '@/app/types/workExpirience'
...
export const ExperienceItem = ({ experience }: ExpirienceItemProps) => {
  const {
    endDate,
    companyName,
    companyLogo,
  ...
  const formartedEndDate = endDate
      ...
      <div>
        ...
        <div className="flex gap-x-2 gap-y-3 flex-wrap lg:max-w-[350px] mb-8">
          {technologies.map((tech, i) => (
            <TechBadge
              key={`expirience-${companyName}-${tech.name}`}
              name={tech.name}
              {...techBadgeAnimation}
